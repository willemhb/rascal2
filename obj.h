...
uint32_t typecode(rval_t);
rtype_t* type(rval_t);
chr_t* vm_val_typename(rval_t);
uint32_t vm_obj_size(rval_t);

#define selfcode(t)  ((t)->head.meta)
#define tp_lowtag(t) ((t)->head.flags)
...
#define _tobvec(v) ((rbvec_t*)ptr(v))
...
#define isbvec(v) (typecode(v) == TYPECODE_BVEC)
