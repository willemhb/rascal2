...
// environments
rcons_t* vm_new_env(rcons_t* n, rcons_t* b, rval_t parent) {
  // TODO: validate n and b
  rcons_t* locals = vm_new_cons(tagcons(n), tagcons(b));
  rcons_t* out = vm_new_list(tagcons(locals), parent);
  return out;
}

rval_t vm_assoc_sym_env(rsym_t* v, rval_t e) {
  int32_t t = typecode(e);
  rcons_t* cr;
  ...
  case TYPECODE_NIL:
    return tagsym(v);
  case TYPECODE_CONS:
    cr = vm_assoc_list(tagsym(v), _tocons(_tocons(e)->car));
    if (cr == NULL) {
  ...
  default:
    eprintf(stderr, "Invalid environment type %d.\n", t);
    escape(ERROR_TYPE);
  }
}

rval_t vm_get_sym_env(rsym_t* v, rval_t e) {
  rval_t result = vm_assoc_sym_env(v,e);
  int32_t t = typecode(result);

  if (t == TYPECODE_NIL) {
    return NONE;
  } else if (t == TYPECODE_CONS) {
    return _tocons(result)->car;
  } else {
    return _tosym(result)->binding;
  }
}


// put_sym_env adds a symbol to the environment
void vm_put_sym_env(rsym_t* s, rval_t e) {
  int32_t t = typecode(e);

  if (t == TYPECODE_NIL) {
    return;
  } else if (t == TYPECODE_CONS) {
    rcons_t* names = _tocons(_tocons(e)->car);
    rcons_t* bindings = _tocons(_tocons(e)->cdr);
    vm_cons_append(names,tagsym(s));
    vm_cons_append(bindings,NIL);
  }
}

// set_sym_env updates the value of an existing symbol
void vm_set_sym_env(rsym_t* s, rval_t v, rval_t e) {
  if (isnil(e)) {
    s->binding = v;
    return;
  }
  rval_t result = vm_assoc_env(s, e);
  _tocons(result)->car = v;
  return;
}
