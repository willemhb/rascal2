...
// pairs
list_t*   mk_list(val_t*,size_t);
list_t*   mk_listn(size_t,...);
...
val_t     pop_list(size_t,val_t*);
bool      last_expression(val_t);
pair_t*   sf_topair(const chr_t*,const int32_t,const chr_t*, val_t*);
list_t*   sf_tolist(const chr_t*,const int32_t,const chr_t*,val_t*);


#define head(p)    ((p)->car)
#define tail(p)    ((p)->cdr)
#define topair(v)  sf_topair(__FILE__,__LINE__,__func__, &(v))
#define tolist(v)  sf_tolist(__FILE__,__LINE__,__func__, &(v))
